...
export interface LogContext {
	...
	userName?: string;
	channelName?: string;
	guildName?: string; // Discord-specific: server name
}
...
function formatContext(ctx: LogContext): string {
	// DMs: [DM:username]
	// Channels: [#channel-name:username] or [guild/#channel:username]
	const isDM = !ctx.guildName;
	if (isDM) {
...
export function logStartup(workingDir: string, sandbox: string): void {
	console.log("Starting mom-discord bot...");
	console.log(`  Working directory: ${workingDir}`);
	console.log(`  Sandbox: ${sandbox}`);
}

export function logConnected(): void {
	console.log("⚡️ Mom Discord bot connected and listening!");
	console.log("");
}

export function logDisconnected(): void {
	console.log("Mom Discord bot disconnected.");
}
